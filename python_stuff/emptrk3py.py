...
class Control:
        def __init__(self, parent):
            ...
            self.choose_btn.grid(row = 5,column = 6)
            self.show_btn.grid(row = 6, column = 6)
            self.data = {}
        ...
        def print_selected_date(self):
            print(self.data)
        def ret_selected_date(self,d):
            d = self.data
            print("ret_selectes",d['month_selected'])
            return(d)
...
#        self = self.Tk()
    ...
    def   main_menu(self):
            ...
            b3 = Button(self, text = "Update", command = lambda self = self:self.update(e7))
    ...
    def update(self,e7):
        ...
        b4 = Button(top, text = "Submit", command = lambda: self.entry_update(c,e7))
        b4.grid(row = 8, column = 6)
        b5 = Button(top, text = "Quit", command= lambda: top.destroy())
        b5.grid(row = 10, column = 6)
        c = Control(top)
        
    def entry_update(self,c,vals):
#  Prepare a statement for an uppdate
        print("entry_update")
        d = c.ret_selected_date(vals)
        print("entry_update",d)
